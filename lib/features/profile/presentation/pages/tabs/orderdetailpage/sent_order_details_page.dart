...
import 'widgets/sent_order_details_actions.dart';
...
class SentOrderDetailsPage extends ConsumerWidget {
  final WholesaleOrder order;

  const SentOrderDetailsPage({super.key, required this.order});

  static void show(BuildContext context, WholesaleOrder order) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => SentOrderDetailsPage(order: order),
      ),
    );
  }
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
        onRefresh: () async {
          await ref.read(sentOrdersProvider.notifier).refresh();
          await ref.refresh(userPdfsProvider.future);
        },
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
                      ...
                      children: [
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              Text(
                                'Order #${currentOrder.id.length > 8 ? currentOrder.id.substring(0, 8) : currentOrder.id}...',
                                style: GoogleFonts.outfit(
                                  fontSize: 24.sp,
              ...
              // ── Action Buttons Bottom: Specialized for Sent Orders ─────────────
              SentOrderDetailsActions(order: currentOrder),
            ],
          ),
        ),
      ),
